...
void
produceNum (void* aux) {
  lock_acquire(&number_lock);
  printf("Here begins Number_producing:\n");
  struct sortNumber* it = NULL;
  int tmp[88], i;
  random_bytes(tmp, sizeof(tmp));
  for (i = 0; i < 88; i++) {
      it = (struct sortNumber *)malloc(sizeof(struct sortNumber));
      tmp[i] %= 88;
      it->number = (tmp[i]);
      list_push_back(&number_list, &it->elem);
  }
  for (i = 0; i < 88; i++) printf("%d ", tmp[i]);
  printf("\n");
  printf("------------------------------------------------------------------\n");
  lock_release(&number_lock);
}

void
printNum (void* aux) {
  // print and free all the struct;

  lock_acquire(&number_lock);
  printf("Here comes the printing\n");
  struct list_elem* e = list_head (&number_list);
  while ((e = list_next (e)) != list_end (&number_list)) {
    printf("%d ", list_entry(e, struct sortNumber, elem)->number);
  }
  printf("\n");
  printf("\n");
  printf("------------------------------------------------------------------\n");
...
bool compare_less_func(struct list_elem* a, struct list_elem* b, void *aux) {
  int numA = list_entry(a, struct sortNumber, elem)->number;
  int numB = list_entry(b, struct sortNumber, elem)->number;
  return (numA > numB);
}
...
sortNum (void* aux) {
  ...
  lock_acquire(&number_lock);
  printf("here runs the sorting!\n");
  list_sort(&number_list, compare_less_func, NULL);
  printf("\n");
  printf("------------------------------------------------------------------\n");
  lock_release(&number_lock);
}
...
void
hello_pintos (void) {
  printf("hello_kinsang!\n");
  list_init(&number_list);
  lock_init(&number_lock);
  thread_create("produce_n", 63, produceNum, NULL);
  thread_create("sort_n", 63, sortNum, NULL);
  thread_create("print_n", 63, printNum, NULL);
  lock_acquire(&number_lock);
  lock_release(&number_lock);
}
