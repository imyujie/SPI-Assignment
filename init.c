...
bool compare_less_func(struct list_elem * a, struct list_elem * b, void *aux) {
	return list_entry(a, struct sortNumber, elem)->number < list_entry(b, struct sortNumber, elem)->number;
}
...
void produceNumber(void* aux) {
	lock_acquire(&number_lock);
	struct sortNumber *l = NULL;
	int t = 0;
	printf("Produce_number: \n");
	for (t = 0; t < 100; t++) {
		l = (struct sortNumber *)malloc(sizeof(struct sortNumber));
		printf("hehehehhehe");
		random_bytes(&(l->number), sizeof(int));
		l->number %= 70;
		printf("%d ", l->number);
		list_push_back(&number_list, &l->elem);
	}
	printf("\n");
	lock_release(&number_lock);
}
...
hello_pintos (void) {
  ...
  list_init(&number_list);
	...
	thread_create("produceNumber", 63, produceNumber, NULL);
	thread_create("sortNumber", 63, sortNumber, NULL);
	thread_create("printNuber", 63, printNumber, NULL);
	lock_acquire(&number_lock);
	lock_release(&number_lock);
}
